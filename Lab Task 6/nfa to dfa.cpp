...
void input_NFA()
{
    int state, symbol, a;
    NFA[0][0].index[0] = '-';
    std::cout<<"Total number of states: ";
    std::cin>>state;
    getchar();
    NFA_r = state;
    std::cout<<"Enter states with a space: ";
    ...
    std::cout<<"Total number of symbols (the labels on transitions between states): ";
    std::cin>>symbol;
    getchar();
    NFA_c = symbol;
    std::cout<<"Enter symbols with a space: ";
    ...
    for(int i = 1 ; i <= NFA_r; i++)
    {
        ...
        {
            char string[10];
            std::cout<<"\nTransitions of { "<<NFA[i][0].index[0]<<" } with symbol { "<<NFA[0][l].index[0]<<" }: \nFor example: A,B,C and press ENTER. OR just press ENTER with no input for no transitions\n>>";
            gets(string);
...
void input_eNFA()
{
    ...
    std::cout<<"Total number of symbol: ";
    std::cin>>symbol;
    ...
    for(int i = 1 ; i <= eNFA_r; i++)
    {
        ...
        {
            char string[10];
            std::cout<<"Transitions of "<<(eNFA[i][0].index[0])<<" with symbol { "<<eNFA[0][l].index[0]<<" }: \nExample: A,B,C and press ENTER or simply press ENTER if there are no transitions"<<std::endl;
            gets(string);
...
void display(){
	for(int j = 0; j < s; j++){
        ...
        std::cout<<"Epsilon closure of "<<states[j]<<" = ";
        for(int i = 0; i < col; i++)
        {
            printf(" %c",result[j][i]);
            if(result[j][i+1] == '\0')
...
int main()
{
    ...
    std::cout<<"\nAre there any epsilon Transition? (y/n): ";
    std::cin>>ch;
    getchar();
